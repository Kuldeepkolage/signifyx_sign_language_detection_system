...
app.post("/api/detect-image", async (req, res) => {
  try {
    ...
    // ✅ Actually call the ML service
    const mlResponse = await axios.post("http://127.0.0.1:5000/predict", {
      image: image
    });

    const detectedLetter = mlResponse.data.gesture || mlResponse.data.detectedSign;
    const confidence = mlResponse.data.confidence || 0.95;
    ...
    res.json({
      success: true,
      predictions: [{ letter: detectedLetter, confidence: confidence }],
      detectedSign: detectedLetter
    });
  } catch (error) {
